...
class CXRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Use source_dir column if available, otherwise use img_dir
        if 'source_dir' in row and pd.notna(row['source_dir']):
            img_path = Path(row['source_dir']) / row['new_filename']
        else:
...
def train_fold(fold, device='cuda'):
    ...
    # Data - using data/fold CSVs which have source_dir column
    train_dataset = CXRDataset(f'data/fold{fold}_train.csv', 'train_images',
                                img_size=384, augment=True)
    val_dataset = CXRDataset(f'data/fold{fold}_val.csv', 'train_images',
