#!/usr/bin/env python3
"""
生成 Layer 1 所有模型的測試集預測
用於 Layer 2 meta-learner 集成
"""
...
def generate_predictions_for_model(checkpoint_path, test_csv, images_dir, output_path,
    ...
    dataset = SimpleDataset(test_csv, images_dir, img_size)
...
def main():
    ...
    for model_type, config in models_config.items():
        ...
        for fold in range(5):
            ...
            # Output file
            output_file = output_dir / f'{model_type}_fold{fold}_test_pred.csv'
    ...
    pred_files = sorted(output_dir.glob('*_test_pred.csv'))
    ...
    if completed_models == total_models and total_models > 0:
        print(f"\n✅ 所有測試集預測已生成！")
        print(f"\n下一步:")
        print(f"  python3 scripts/stacking_predict.py")
    else:
        print(f"\n⚠️ 部分模型預測失敗，請檢查錯誤")
